...
variable "uptrace_event_type" {
  type        = string
  description = "Either metrics or logs."
}
...
variable "s3_failure_bucket_arn" {
  type        = string
  description = "A name of the S3 bucket that will store any logs that failed to be sent to Uptrace."
}
